<template>
  ...
  <el-table :data="tableData" stripe>
    ...
    <el-table-column label="操作" width="230">
      <template v-slot="scope">
        <el-button type="primary" size="mini" @click="$router.push('/editAdmin?id=' + scope.row.id)">编辑</el-button>
        <el-popconfirm
        ...
        >
          ...
          <el-button style="margin-left: 5px" type="warning" size="mini" @click="handleChangePass(scope.row)">修改密码</el-button>
      </template>
    </el-table-column>
  </el-table>
          ...
          request.put('/admin/password', this.form).then(res => {
            if (res.code === '200') {
              ...
              if (this.form.id === this.admin.id) { //当前修改的用户id等于当前登录的管理员id，那么修改成功之后需要重新登录
                Cookies.remove('admin') //清除掉当前的用户信息然后重新登录
